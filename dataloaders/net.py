...
class ProcessModelDataset(Dataset):
    ...
    def _generate_configurations(self):
        ...
        for method_name, fn in self.discovery_methods.items():
            # Get grid for this method
            method_grid = self.param_grid.get(method_name, self.param_grid)
            if not isinstance(method_grid, dict):
                raise TypeError(
                    f"param_grid must be dict or nested dict, got {type(method_grid)}"
                )
            ...
            for params in method_param_combos:
                # Remove params not in signature
                params = {k: v for k, v in params.items() if k in sig_params}
                subset = self.sampler_fn(self.log)
                configs.append((method_name, fn, params, subset))
...
class PARAM_GRID(Enum):
    STANDARD = {
        "noise_threshold": torch.linspace(0.0, 1.0, 10).tolist(),
        "dependency_threshold": [0.5, 0.7],
    }
...
if __name__ == "__main__":
    ...
    pm_dataset = ProcessModelDataset(
        ...
        discovery_methods=DISCOVERY_METHODS.ALL.value,
        param_grid=PARAM_GRID.STANDARD.value,
