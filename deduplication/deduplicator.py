...
class PetriNetDeduplicator:
    ...
    def __init__(self, config: DeduplicationConfig):
        ...
        edge_comparator = PathBasedTransitionEdgeComparator(
            debug_callback=edge_callback
        )
        feature_comparator = DualScoreFeatureComparator(
            debug_callback=feature_callback
        )
    ...
    def deduplicate(
        ...
        for current_net in tqdm(
            nets, disable=not self.config.verbose, desc="Deduplicating"
        ):
            is_duplicate, representative_idx = self._find_duplicate(
                current_net, unique_nets
            )
    ...
    def _find_duplicate(
        self, candidate: PetriNetItem, unique_nets: List[PetriNetItem]
    ...
    def _generate_report(
        self, unique_nets: List[PetriNetItem], duplicate_map: Dict[int, int]
    ):
        """
    ...
    def print_report(self):
        ...
        if self.report is None:
            print(
                "No deduplication report available. Run deduplicate() first."
            )
            return
        ...
        print(
            f"Reduction:                   {self.report['reduction_percent']:.1f}%"
        )
        print()
        print("COMPARISON STATISTICS:")
        print(
            f"  Total comparisons:         {self.report['stats']['comparisons_performed']}"
        )
        print(
            f"  Passed stage 1 (labels):   {self.report['stats']['stage1_filtered']}"
        )
        print(
            f"  Passed stage 2 (combined): {self.report['stats']['stage2_filtered']}"
        )
        print()
        print("THRESHOLDS:")
        print(
            f"  Label similarity:          >= {self.report['thresholds']['label_threshold']:.2f}"
        )
        print(
            f"  Combined similarity:       >= {self.report['thresholds']['combined_threshold']:.2f}"
        )
        print()
        print("STAGES ENABLED:")
        print(
            f"  Stage 1 (prefilter):       {self.report['stages_enabled']['stage1']}"
        )
        print(
            f"  Stage 2 (combined):        {self.report['stages_enabled']['stage2']}"
        )
        print("=" * 70 + "\n")
