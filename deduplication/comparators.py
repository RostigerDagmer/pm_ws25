...
from features.extractors import BaseFeatureExtractor, ModelFeatureExtractor
from typing import *
...
class TransitionLabelComparator(BaseComparator):
    ...
    def __init__(
        self, use_cache: bool = True, debug_callback: Optional[Callable] = None
    ):
        """
    ...
    def _extract_label_counts(self, net: PetriNet) -> Counter:
        ...
        for transition in net.transitions:
            ...
            if label is not None:  # Ignore invisible transitions
                counts[label] += 1
    ...
    def compare(
        ...
        if not all_labels:
            ...
            numerator = sum(
                abs(counts1[label] - counts2[label]) for label in all_labels
            )
            denominator = sum(
                counts1[label] + counts2[label] for label in all_labels
            )
...
class DualScoreFeatureComparator(BaseComparator):
    ...
    def _canberra_distance(
        self, x: np.ndarray, y: np.ndarray, indices: list, weights: np.ndarray
    ) -> float:
        """
