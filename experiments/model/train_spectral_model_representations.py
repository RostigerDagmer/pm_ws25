...
def op_dist_grid():
    t = torch.tensor(list(product(*([torch.linspace(0.1, 0.8, 6).tolist()] * 4))), dtype=float)
...
@torch.no_grad()
def enabled_trans_to_vocab(enabled_hist: torch.Tensor,
                           local_to_vocab: torch.Tensor,
    ...
    try:
        for epoch in range(epochs):
            ...
            for idx in torch.randperm(len(synthetic_dataset)):
                ...
                model_basis, embedded_log, targets, local_to_vocab = prepare_masked_batch(
                ...
                logits = model(batch_model_basis, embedded_log, trace_mask=trace_mask)  # [B,S,V]
                ...
                B, S = tok_idx.shape
                pos_mask = (torch.arange(S, device=device).unsqueeze(0) < lengths.unsqueeze(1))
                pos_mask &= (tok_idx != -1)
                ...
                if global_step % log_every == 0:
                    ...
                    writer.add_scalar("train/loss_mlm", mlm_loss.item(), global_step)
                    writer.add_scalar("train/loss_feas", feas_loss.item(), global_step)
                    writer.add_scalar("train/lr", scheduler.get_last_lr()[0], global_step)
                    writer.add_scalar("train/total_seq_tokens", total_seq_tokens, global_step)
                    writer.add_scalar("train/total_net_tokens", total_net_tokens, global_step)
                    writer.add_scalar("train/total_tokens", total_seq_tokens + total_net_tokens, global_step)

                    # timings (ms)
                    writer.add_scalar("time/sim_ms", (t_sim1 - t_sim0) * 1000.0, global_step)
                    writer.add_scalar("time/prepare_ms", (t_prep1 - t_prep0) * 1000.0, global_step)
                    writer.add_scalar("time/forward_ms", (t_fwd1 - t_fwd0) * 1000.0, global_step)

                    # simple stats
                    avg_len = lengths.float().mean().item()
                    writer.add_scalar("data/avg_seq_len", avg_len, global_step)
                    writer.add_scalar("data/vocab_size", logits.size(-1), global_step)
            ...
            # epoch aggregates
            if epoch_steps > 0:
                writer.add_scalar("epoch/loss_total", epoch_loss_sum / epoch_steps, epoch)
                writer.add_scalar("epoch/loss_mlm", epoch_mlm_sum / epoch_steps, epoch)
                writer.add_scalar("epoch/loss_feas", epoch_feas_sum / epoch_steps, epoch)
            writer.add_scalar("epoch/duration_s", t_epoch_end - t_epoch_start, epoch)

            # checkpoint per epoch
            ckpt_path = log_dir / f"spectral_model_pretrained_epoch{epoch+1}.pth"
            torch.save(model.state_dict(), ckpt_path)

        # final checkpoint
        torch.save(model.state_dict(), log_dir / "spectral_model_pretrained_final.pth")
