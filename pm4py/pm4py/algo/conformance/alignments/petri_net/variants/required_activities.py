...
def _precompute_required_model_labels(
    sync_net, final_marking
) -> Dict[Any, Set[str]]:
    ...
    while queue:
        place = queue.popleft()
        for t in incoming_transitions.get(
            place, ()
        ):  # transitions producing 'place'
...
class RequiredActivitiesMode(Enum):
    ...
    STRUCTURAL_SHORTEST_PATH = (
        "structural_shortest_path"  # place-level shortest path
    )
...
def _heuristic_simple_required_activities(
    ...
    idx_candidates = [
        place_to_trace_index[p] for p in marking if p in place_to_trace_index
    ]
    if idx_candidates:
    ...
    while queue:
        ...
        for t in incoming_transitions.get(
            ...
            for arc in t.in_arcs:
                ...
                if model_label is not None:
                    if (
                        model_label not in by_label
                        or add_cost < by_label[model_label]
                    ):
...
def _precompute_bitset_structures(
    ...
    place_to_required: Dict[Any, Set[str]] = _precompute_required_model_labels(
        sync_net, final_marking
    )
...
def _precompute_structural_shortest_path(
    ...
    for _ in range(max_iters):
        ...
        for t in transitions:
            ...
            succ_vals = [
                dist_place.get(arc.target, float("inf")) for arc in t.out_arcs
            ]
            if not succ_vals:
                continue
            d_out = max(succ_vals)
...
def __search(
    ...
    mode_val = exec_utils.get_param_value(
        Parameters.REQUIRED_ACTIVITIES_MODE,
        parameters or {},
        RequiredActivitiesMode.SIMPLE.value,
    )
    try:
    ...
    if mode in {
        ...
        place_to_required_labels = _precompute_required_model_labels(
            sync_net, fin
        )
