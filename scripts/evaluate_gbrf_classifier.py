"""
Evaluation script for Gradient Boosting and Random Forest classifiers.
...
5. Trains GradientBoostingClassifier and RandomForestClassifier on train datasets
6. Evaluates on test datasets and compares with baselines (including XGBoost)
"""
...
SEED = 1
OUTPUT_DIR = Path("outputs") / "evaluate_gbrf_classifier"
...
if __name__ == "__main__":
    ...
    # Train baselines (including XGBoost)
    ...
    logging.info(f"\n{'='*60}")
    logging.info("Evaluating Gradient Boosting Classifier on {len(test_tables)} test datasets...")
    logging.info(f"{'='*60}")
    gb_evaluator = RecommenderEvaluator(
        classifier=gb_classifier, tables=test_tables
    )
    gb_metrics = gb_evaluator.evaluate()

    # Evaluate Random Forest classifier
    logging.info(f"\n{'='*60}")
    logging.info("Evaluating Random Forest Classifier on {len(test_tables)} test datasets...")
    logging.info(f"{'='*60}")
    rf_evaluator = RecommenderEvaluator(
        classifier=rf_classifier, tables=test_tables
    )
    rf_metrics = rf_evaluator.evaluate()

    # Compare with all baselines (including XGBoost)
    logging.info("\n" + "="*60)
    logging.info("Gradient Boosting Comparison with Baselines")
    logging.info("="*60)
    gb_comparison_df = gb_evaluator.compare_with_baselines([xgb_classifier, single_best, random_clf])
    logging.info("\n" + gb_comparison_df.to_string())

    logging.info("\n" + "="*60)
    logging.info("Random Forest Comparison with Baselines")
    logging.info("="*60)
    rf_comparison_df = rf_evaluator.compare_with_baselines([xgb_classifier, single_best, random_clf])
    logging.info("\n" + rf_comparison_df.to_string())

    # Save Gradient Boosting results
    RecommenderEvaluator.save_results(
        metrics=gb_metrics,
        comparison_df=gb_comparison_df,
    ...
    RecommenderEvaluator.save_results(
        metrics=rf_metrics,
        comparison_df=rf_comparison_df,
        output_dir=OUTPUT_DIR / "random_forest",
        train_count=len(train_tables),
        test_count=len(test_tables),
    )

    # Create combined comparison summary
    logging.info("\n" + "="*60)
    logging.info("Combined Model Comparison Summary")
    logging.info("="*60)

    combined_summary = pd.DataFrame({
        'Model': ['Gradient Boosting', 'Random Forest', 'XGBoost', 'SingleBest', 'Random'],
        'Accuracy': [
            gb_metrics.tolerance_metrics[0.0].overall_accuracy,
            rf_metrics.tolerance_metrics[0.0].overall_accuracy,
            gb_comparison_df[gb_comparison_df['Model'] == 'XGBoostClassifier']['Accuracy'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'SingleBestSolver']['Accuracy'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'RandomClassifier']['Accuracy'].values[0],
        ],
        'Performance Ratio': [
            gb_metrics.performance_ratio_alignment_only,
            rf_metrics.performance_ratio_alignment_only,
            gb_comparison_df[gb_comparison_df['Model'] == 'XGBoostClassifier']['Performance Ratio'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'SingleBestSolver']['Performance Ratio'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'RandomClassifier']['Performance Ratio'].values[0],
        ],
        'Time Savings': [
            gb_metrics.time_savings_vs_worst,
            rf_metrics.time_savings_vs_worst,
            gb_comparison_df[gb_comparison_df['Model'] == 'XGBoostClassifier']['Time Savings'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'SingleBestSolver']['Time Savings'].values[0],
            gb_comparison_df[gb_comparison_df['Model'] == 'RandomClassifier']['Time Savings'].values[0],
        ],
    })
    ...
    combined_output_path = OUTPUT_DIR / "combined_summary.csv"
    combined_output_path.parent.mkdir(parents=True, exist_ok=True)
    combined_summary.to_csv(combined_output_path, index=False)
